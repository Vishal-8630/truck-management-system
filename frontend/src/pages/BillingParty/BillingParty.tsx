  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (addBillingPartyAsync.fulfilled.match(resultAction)) {
        ...
        dispatch(
          addMessage({ type: "error", text: errors?.message || "Please fill all the required fields" })
        );
      }
    } catch {
      ...
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8">
        <div className="flex flex-col gap-2">
          <h1 className="text-4xl lg:text-5xl font-bold tracking-tight text-slate-900 leading-tight italic flex items-center gap-4">
            <Users className="text-indigo-600 w-10 h-10 lg:w-12 lg:h-12" />
            Billing <span className="text-indigo-600">Parties</span>
          </h1>
